 ...
 *
 * tips：leetCode官方解法中定义了一个查找第k小的数的函数，通过调用该方法找到中位数本数(奇数)或者第k和k+1小的数(偶数)
 *       但是实际写的过程中发现偶数情况在找第k小的数时可以顺带将第k+1小的数一起找出来额外多出来的操作只需要常数项的复杂度就可解决
 *       算是一种优化，不过代码。
 *
 * 本题最优解 ：
 *
...
public class P4_FindMedianInTwoSortedArrays {
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while (true) {
            ...
            if (offset1 == len1) {
                if ((len1 + len2) % 2 == 0) {
                    return (nums2[offset2 + k - 1] + nums2[offset2 + k]) / 2.0;
                } else {
            ...
            if (offset2 == len2) {
                if ((len1 + len2) % 2 == 0) {
                    return (nums1[offset1 + k - 1] + nums1[offset1 + k]) / 2.0;
                } else {
            ...
            if (k == 1) {
                ...
                if (nums1[offset1] < nums2[offset2]) {
                    ...
                    if (offset1 + 1 == len1) {
                        return (nums1[offset1] + nums2[offset2]) / 2.0;
                    } else {
                        return (nums1[offset1] + Math.min(nums2[offset2], nums1[offset1 + 1])) / 2.0;
                    }
                    ...
                    if (offset2 + 1 == len2) {
                        return (nums2[offset2] + nums1[offset1]) / 2.0;
                    } else {
                        return (nums2[offset2] + Math.min(nums1[offset1], nums2[offset2 + 1])) / 2.0;
                    }
                } else {
                    return nums1[offset1];
                }
